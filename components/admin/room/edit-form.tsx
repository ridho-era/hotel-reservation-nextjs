...
import { RoomProops } from "@/types/room";
...
const EditForm = ({
  ...
  const handleUpload = () => {
    if (!inputFileReff.current?.files) return null;
    const file = inputFileReff.current.files[0];
    ...
    <form action={formAction}>
      <div className="grid md:grid-cols-12 gap-5">
        ...
        <div className="col-span-4 bg-white p-4">
          <label
            htmlFor="input-file"
            className="flex flex-col mb-4 items-center justify-center aspect-video border-2 border-gray-50 relative"
          >
          ...
          <button
            ...
            className={clsx(
              ...
              {
                "opacity-50 cursor-progress": isPending || !image,
              }
            )}
            disabled={isPending || !image}
          >
            {isPending ? "Updateing..." : "Update"}
          </button>
        </div>
      </div>
    </form>
